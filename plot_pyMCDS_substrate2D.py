"""
  plot_pyMCDS_substrate2D.py - quick & dirty plots of 2D substrates using matplotlib

  Usage: (from an /output directory)
  python plot_pyMCDS_substrate2D.py xml_idx substrate_name 
...
if (len(sys.argv) < 3):
  print("Usage: " + sys.argv[0] + " xml_idx substrate_name")
...
#-----------------------------------------------------
def plot_substrate():
    global current_idx, axes_max, cbar, substrate_name

    print('plot_substrate(): current_idx=',current_idx)
    ...
    # hrs = mins/60.
    # days = hrs/24.
    # print("days,hrs,mins=",days,hrs,mins)
    # title_str = '%d days, %d hrs, %d mins' % (int(days),(hrs%24), int(mins - (int(hrs)*60)))
    hrs = int(mins/60)
    days = int(hrs/24)
    ...
    xml_fname = "output%08d.xml" % current_idx
    # mcds = pyMCDS('output00000087.xml', '~/path/to/PhysiCell/output')
    mcds = pyMCDS(xml_fname, output_dir_str)
    # print("mcds.data= ",mcds.data)
    xgrid, ygrid = mcds.get_2D_mesh()
    print("xgrid= ",xgrid)
    print("ygrid= ",ygrid)

    # print('plot_substrate: field_idx=',field_idx)
    # f = M[field_idx,:]   # 
    M = mcds.get_concentrations(substrate_name)

    # original via scipy: e.g.,  (6, 421875)  (where 421875=75*75*75)
    print("M.shape = ",M.shape)   # M.shape =  (2, 2, 1)
    print("M = ",M)
    print("M[:,:,0] = ",M[:,:,0])
...
#    my_plot = plt.contourf(xvec,xvec,M[field_idx,:].reshape(N,N), num_contours, cmap='viridis') #'viridis'
    if fix_cmap > 0:
      my_plot = plt.pcolormesh(xgrid, ygrid, M[:,:,0], shading='auto',cmap=cmap,vmin=40.,vmax=42.)
    else:
    #   my_plot = plt.contourf(xgrid, ygrid, M[:,:,0], levels=levels, cmap=cmap)
    #   my_plot = plt.contourf(xgrid, ygrid, M[:,:,0], cmap=cmap,vmin=40.,vmax=43.)
      my_plot = plt.contourf(xgrid, ygrid, M[:,:,0], cmap=cmap)

       # shading= 'auto'()='fix') or 'gouraud'
    #   my_plot = plt.pcolormesh(xgrid,ygrid, M[:,:,0], shading='auto', cmap='viridis') #, vmin=Z.min(), vmax=Z.max())
...
step_value = 1
def press(event):
  global current_idx, step_value, substrate_name
